...
real(dp), allocatable, dimension(:,:) :: soilpar
...
! Parameters
real(dp), parameter :: ombd = 0.224  !bulk density of organic matter (g cm-3)
real(dp), parameter :: omcf = 1.724  !conversion factor from organic carbon to organic matter
...
real(dp), allocatable, dimension(:) :: zpos
real(dp), allocatable, dimension(:) :: OrgM  !(g m-2)
real(dp), allocatable, dimension(:) :: whc
real(dp), allocatable, dimension(:) :: Ksat

real(dp) :: Csoil     !(g m-2)
real(dp) :: orgC      !(mass %)
real(dp) :: soilmass  !(g m-3)
real(dp) :: blk0      !(g m-3)
real(dp) :: dOM       !change in organic matter (g m-2)
real(dp) :: dzOM      !interlayer transport of SOM (g m-2)
real(dp) :: dzx       !excess change in top layer thickness
...
integer,  intent(in) :: d
! real(dp), intent(in) :: mat20  !20 year running mean annual temperature
real(dp), intent(in) :: dpet
! real(dp), intent(in) :: dmelt
real(dp), intent(in) :: dprec
! real(dp), intent(in) :: ksat  !(mm hr-1)
! real(dp), intent(in) :: awc
real(dp), dimension(:), intent(in) :: fpc_grid
logical, dimension(:),  intent(in) :: present
real(dp), dimension(:), intent(in) :: dphen  !phenological state (0=no leaves, 1=full canopy)
real(dp), dimension(:), intent(in) :: dgp    !non water stressed canopy conductance (?)

! real(dp), intent(inout), dimension(:) :: w

real(dp), intent(out) :: daet
real(dp), intent(out) :: drunoff_surf
real(dp), intent(out) :: drunoff_drain
real(dp), dimension(:), intent(out) :: dwscal
real(dp), dimension(:), intent(out) :: dgc    !actual canopy conductance (?)

! Parameters
real(dp), parameter :: emax   = 5.   !maximum daily transpiration rate (mm/day)
real(dp), parameter :: alpham = 1.4  !maximum Priestly-Taylor coefficient
real(dp), parameter :: gm     = 5.   !scaling conductance (mm s-1)?

! Local variables
real(dp), dimension(2) :: rootprop
real(dp), dimension(2) :: ksat
real(dp), dimension(2) :: awc
integer :: pft
...
real(dp), dimension(2) :: aettotal
real(dp), dimension(2) :: beta

real(dp), dimension(npft) :: aet
...
real(dp), intent(in),    dimension(2) :: awc       !soil available water content (mm or kg)
real(dp), intent(in),    dimension(2) :: ksat      !saturated conductivity (mm h-1)

! real(dp), intent(in)                  :: melt      !daily total snowmelt (mm)
real(dp), intent(in)                  :: prec      !daily total precipitation (mm)

real(dp), intent(in),    dimension(2) :: aettotal  !total water removed through evapotranspiration from each soil layer (mm)

real(dp), intent(inout), dimension(2) :: swf       !water status in each soil layer (fraction of awc)

real(dp), intent(out)                 :: runoff    !surface runoff (mm)
real(dp), intent(out)                 :: drainage  !groundwater drainage (mm)
...
real(dp), parameter :: dt = 1. / real(ts)
real(dp), parameter :: a  = 1.e-5  !minimum unsaturated conductivity (mm hr-1)

!local variables

integer  :: t         !counters
real(dp) :: qsurf     !surface flux of water (mm h-1)
real(dp) :: perc      !percolation of soil water (mm) between layers
real(dp) :: reversef  !reverse flow: if lower layer is saturated, water backs up into the upper layer

real(dp) :: infil


real(dp), dimension(2) :: whcmm   !water holding capacity of the soil in each layer (mm)
real(dp), dimension(2) :: kmmh    !saturated conductivity of the soil in each layer (mm hr-1)
real(dp), dimension(2) :: swater  !soil water content (mm)
real(dp), dimension(2) :: aet     !evapotranspiration (mm h-1)
