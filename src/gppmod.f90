...
use pftparmod, only : npft,pftpar,c4
...
real(sp), pointer, dimension(:) :: lambda               ! Actual ratio of lead internal to external CO2 (fraction)
real(sp), pointer, dimension(:) :: gammastar            ! Photorespiratory compensation point (gammastar in Wang et al. 2017) (Pa)
real(sp), pointer, dimension(:) :: gpp0                 ! Gross primary productivity under non-water stressed condition (g C m-2 d-1)
real(sp), pointer, dimension(:) :: gpp1                 ! Gross primary productivity under actual condition (g C m-2 d-1)
real(sp), pointer, dimension(:) :: gpp_tot              ! Total daily gross primary productivity (g C d-1)
real(sp), pointer, dimension(:) :: rd                   ! Daily leaf respiration (gC m-2 d-1) >> whole day include day + night
real(sp), pointer, dimension(:) :: dgp                  ! Optimal daily canopy conductance (mm m-2 s-1) ??? NEED CONFIRMATION ON UNIT in LPJ-LMFire
real(sp), pointer, dimension(:) :: dgc                  ! Actual daily canopy conductance (mm m-2 s-1) ??? NEED CONFIRMATION ON UNIT in LPJ-LMFire
...
ca = 2.88e-4
par = srad * 0.5 * 1000.      ! Convert from kJ to J and divide by half to get PAR
tsecs = 3600. * dayl
...
do pft = 1, npft
  ...
  if (present(pft)) then
    ...
    if (dwscal_ytd > minwscal) then ! .and. leafon
      dphen_w = 1.
      dphen = dphen_t
...
do pft = 1, npft
  ...
  ! if (lprint .and. grid==gprint .and. pft == npft) write(0,*) 'pft: ', pft, gpp1, fpc_grid, xmid

end do ! PFT loop



! if (lprint .and. grid==gprint) print *, day, prec, dphen, dwscal_ytd, dwscal, dgp, dgc, w(grid,:), xmid, agd0, agd1
